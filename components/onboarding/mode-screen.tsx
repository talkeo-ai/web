 ...
 *
 * Two answers stacked in one column, the same shape the landing uses to ask
 * its own question — and for the same reason. Side by side they read as
 * equivalent; stacked, with one solid and one outlined, the pair says which
 * one to take without a word of copy spent on saying it. Speaking is the one
 * we want, so speaking is the solid one and it goes on top.
 *
 * Choosing is submitting. There is no Continue after, because there is
  ...
  return (
    ...
    >
      {/* Both share one width rather than sizing to their own labels: stacked
          controls of different widths read as a list of links, and a fixed
          column reads as a decision with two answers. */}
